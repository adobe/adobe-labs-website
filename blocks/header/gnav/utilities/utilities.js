...
import { replaceKey, replaceText, fetchPlaceholders } from '../../milo-compat.js';
import { PERSONALIZATION_TAGS, FLAGS, handleCommands } from '../../milo-compat.js';
...
/**
 * Builds a .plain.html URL, stripping hashes such as #_inline.
 * @param {string} url
 * @returns {string}
 */
function toPlainHtmlUrl(url) {
  let href = url || '';
  try {
  ...
  if (href.endsWith('.plain.html')) return href;
  return href.replace(/(\.html)?$/, '.plain.html');
}
 ...
 */
async function fetchPlainHtml(url, plainHTMLPromise) {
  if (plainHTMLPromise) {
    try {
      const res = await plainHTMLPromise;
...
export async function fetchAndProcessPlainHtml({
  ...
  const res = await fetchPlainHtml(path, plainHTMLPromise);
  if (!res) {
    lanaLog({
  ...
  if (inlineFrags.length) {
    await Promise.all(inlineFrags.map(async (link) => {
      try {
        const href = await localizeLinkAsync(
          getFederatedUrl(link.href || link.getAttribute('href')),
          window.location.hostname,
          false,
          link,
        );
        const fragBody = await fetchAndProcessPlainHtml({ url: href });
        if (!fragBody) return;
        link.replaceWith(...fragBody.childNodes);
      } catch (e) {
        lanaLog({
          message: 'Error loading inline fragments in gnav',
          e,
          tags: 'utilities',
          errorType: 'i',
        });
...
const getAnalyticsValue = async (str, index) => {
  processTrackingLabels = processTrackingLabels ?? (await import('../../milo-compat.js')).processTrackingLabels;
