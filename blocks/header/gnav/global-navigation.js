...
const cssPromise = (async () => {
  ...
  try {
    // EDS loadBlock styles header.css, not this sheet. Load it here.
    await Promise.all([
      loadStylePromise(`${url}base.css`),
      loadStylePromise(`${url}global-navigation.css`),
    ]);
  } catch (e) {
...
const decorateSignIn = async ({ rawElem, decoratedElem }) => {
  ...
  const authoredLink = rawElem.querySelector('a[href]');
  const authoredLabel = authoredLink?.textContent?.trim();
  const signInLabel = authoredLabel || await replaceKey('sign-in', getFedsPlaceholderConfig());
  const canUseIms = typeof window.adobeIMS?.signIn === 'function';
  let signInElem;

  if (!dropdownElem) {
    if (authoredLink?.href && !canUseIms) {
      signInElem = toFragment`<a href="${authoredLink.href}" class="feds-signIn button" daa-ll="${signInLabel}">${signInLabel}</a>`;
    } else {
    ...
    const dropdownSignInAnchor = dropdownElem.querySelector('[href$="?sign-in=true"]') || authoredLink;
    if (dropdownSignInAnchor && !canUseIms) {
      dropdownSignInAnchor.classList.add('feds-signIn');
    } else if (dropdownSignInAnchor) {
...
class Gnav {
  constructor({ content, block, newMobileNav } = {}) {
    ...
    this.blocks = {
      profile: {
        rawElem: this.content.querySelector('.cta, .profile'),
  ...
  ims = async () => (window.adobeIMS?.initialized ? this.imsReady() : loadIms()
    ...
    .catch(async (e) => {
      ...
      // IMS is out of scope; still paint a CMS-authored CTA from the cta/profile block.
      if (e?.message === 'IMS disabled') {
        const { rawElem, decoratedElem } = this.blocks.profile || {};
        if (rawElem && decoratedElem) await decorateSignIn({ rawElem, decoratedElem });
        return;
      }
  ...
  loadDelayed = async () => {
    ...
    // Search and profile modules are not ported in this initial migration.
  ...
  decorateGenericLogo = ({ selector, classPrefix, includeLabel = true, analyticsValue } = {}) => {
    ...
    const getImageEl = () => {
      const svgImgs = rawBlock.querySelectorAll('picture img[src$=".svg"], img[src$=".svg"]');
      if (isDarkMode()) {
        if (svgImgs.length >= 2) return svgImgs[1];
      ...
      if (svgImgs.length) return svgImgs[0];
    ...
    const returnValue = await (async () => {
      switch (itemType) {
        ...
        case 'asyncDropdownTrigger': {
          ...
          // EDS .plain.html fragments do not get a `.section` class; large-menu is the mega-menu.
          const isSectionMenu = item.closest('.section, .large-menu') instanceof HTMLElement;
        ...
        case 'secondaryCta': {
          ...
          return addMepHighlightAndTargetId(toFragment`<div class="feds-navItem feds-navItem--centered" role="listitem">
              ${decorateCta({ elem: ctaLink.classList.contains('merch') ? await (await import('../milo-compat.js')).default(ctaLink) : ctaLink, type: itemType, index: index + 1 })}
            </div>`, item);
        }
        case 'link': {
          ...
          if (linkElem.classList.contains('merch')) {
            linkElem = await (await import('../milo-compat.js')).default(linkElem);
          }
        ...
        case 'text': {
          ...
          const content = isMerch ? await (await import('../milo-compat.js')).default(item) : item.textContent;
