...
- The last rounded section overlaps the footer by the section radius. Footer inner padding grows by that amount so links stay clickable (`.footer` uses `z-index: 0`). Footer start padding increases at 64rem and above. The Adobe logo does not animate: `.footer__logo` sticks to the bottom of the viewport, and the footer content scrolling past it is the whole reveal. When motion is opted in, `.footer__inner` sticks to the bottom behind the last rounded card, clips, and translates its content as the card uncovers it. While the card covers it outright, the menu's links and newsletter field leave the tab order, the same way a fully covered card's controls do. After the menu is in, the logo sticky takes over. Reduced motion keeps the radius peek.
